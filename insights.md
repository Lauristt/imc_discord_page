...
- Robot_Dishes产品价格在±100范围内波动，有85%的per-tick回归率，仅在突破10k时出现该模式
- sum5为常数
- 针对Robot_Dishes的策略：做市，并在异常波动给出比spread更好价格时主动吃单，单日收益约20k（day4达400k）
- 做市填充率低（约3%）属正常，报价过远会进一步降低fill率
...
- 检查lead-lag关系是数据分析的常规步骤（在一阶差分相关性之后），用50的lead-lag参数不算硬编码
- 单日PnL上限大约在60-70k左右
...
- 多策略组合的关键在于干净表达信号：pair RV、选择性lead-lag、basket关系、静态卫星策略，配合优先级/状态管理避免相互冲突
- 单纯叠加趋势跟随、波动率、MR等会让PnL下沉（曾到-22K），"初始参数+调整参数"方式也失败
- 良好的报价/执行（quoting/ownership）对发挥alpha至关重要，差的执行会抹掉好的信号
- R3策略：使用期权作为杠杆（买入所有期权），并用机器人执行获得更好的被动成交和对冲
- R4配对/篮子策略加上少量MR可以做到10名左右

---

## 2026-04-30 06:20

- R4中发现mark 38和mark 55存在可利用的偏离midprice的成交机会，但难以预测发生时机
- 期权与标的同向移动，若能系统性预测标的走势，可同时买入期权放大收益
- 均值回归策略的执行原则：尽量减少可调超参数数量
...
- Pebbles 50K和Microchips的关系信号被讨论：有选手暴力搜索后发现关系存在，但偏离度对比spread后认为实用价值有限，即使做market making也难以转化为显著alpha
- 关于超参数稳健性的讨论：策略只要超参数有依据且不过度敏感就可接受

NO_INSIGHTS（其余多为闲聊和提交时间疑问）

---

## 2026-04-30 06:25

- Mark 38 在 hydrogel 上是活跃交易者：清掉某一侧流动性后，在中间价上下 99 报价，他每次都会成交
- Mark 55 在 velvetfruit 上同样模式：清掉流动性后在中间价上下 51 报价可被对方吃单
- 该 exploit 在 R1 和 R2 也存在，但无法预测触发时点

---

## 2026-04-30 06:27

- R4中如果订单簿一侧为空，bots会以远离mid的价格成交（R2也有类似现象）
- R4的考点应是分析具名bots已经做出的交易；但trader_id只在实际成交时出现，下一个timestamp才能看到，难以实时利用
- 暴力搜索相关性容易发现大量虚假相关，关键在于将信息转化为可盈利策略

---

## 2026-04-30 06:30

- #algo-trading: 有选手提到taker bots会接受极端价格，可以通过挂极端价格订单从bot处获利（但仅在IMC回测中可行，因为可硬编码交易时点，实盘无效）
- #algo-trading: 建议使用Johansen协整检验快速发现产品间关系（如Pebbles相关性）
...
- Round5中,trader Mark 38在除sleep pods和microchips外的所有产品上,以与R3/R4中taker trades相同的时间、日期和数量同时挂买卖单(可能是套利信号但难以预测)
- 一种策略:在inside bid的最高价或对手平均spread处挂被动单,有时会成交;理论上可在极端价位(如99)挂单等待成交,但需先清空订单簿才能利用,否则不盈利
- 试图预测这些特殊trade耗费大量时间,可能导致bug增加和PnL损失,可能并非有意设计的可利用机制

---

## 2026-04-30 06:37

- R5中microchips策略：跟踪circle的收益率，等待后根据信号调整fair价格上下偏移
- R5中pebbles策略：利用三个pebbles产品fair价格之和等于50k的关系
- 因pebbles同时交易，可通过做市(MM)方式从sum=50k约束中获利
- 该规律在R1-R4都存在，并非R5新引入

---

## 2026-04-30 06:40

- 8个产品（pebbles、chips等）在同一tick交易，总和固定为50k
- 利用总和固定50k的性质：估算XL公允价 = 50k - sum(其他产品)，本地回测3天赚75k
- 由于产品总和固定50k且都是市场吃单方，纯做市基本不会亏损，spread较大
- 纯做市策略在所有pebbles上总共赚约30k；基于随机游走+50k总和约束的概率跳跃均值回归策略效果更好
- 有人提到"lead leg"概念（可能某个产品领先其他产品价格变动）

---

## 2026-04-30 06:42

- 期权/波动率策略依赖于波动率regime保持不变（历史数据上各天波动率较为稳定）
- 概率性均值回归思路：不使用固定均值，而是根据该资产的波动率分布动态切换到新的均值，假设新均值会作为局部趋势维持

---

## 2026-04-30 06:47

- 部分产品方差极高（如pebbles XL），运气好单一产品可能赚30万
- 主流策略包括均值回归/z-score交易、领先滞后、做市等
- 协整（cointegration）在回测中表现尚可，但容易过拟合，需谨慎
- 实盘测试了大量3:1和4:1的协整配对，部分p值低至0.0001

---

## 2026-04-30 06:55

- Round 1中宽价差成交仅在订单簿单侧为空，或在已知即将有交易时清空订单簿才可能实现；Round 2之后无法预测交易，因此该策略不再可用

---

## 2026-04-30 07:45

- algo-trading频道：有选手反馈lead-lag策略效果不佳，主要PnL来源于均值回归、z-score排序偏差、宽价差做市和部分动量策略
- algo-trading频道：提及代码字符数限制可能为50000

---

## 2026-04-30 08:13

- 学习游戏理论建议：研究赛后发布的writeup并自己动手实现代码
- 算法策略思路：不要试图学习交易bot的行为，而是通过log文件统计分析各item的特性（包括spread、流动性等交易行为），然后针对性地组合（ensemble）策略来匹配这些特性

---

## 2026-04-30 09:13

- Round相关策略讨论：基于回归线做均值回归，用偏离的标准差作为触发信号，仓位按偏离程度缩放（回测约1.3M）
- 已知的alpha来源：lead-lag关系、做市(MM)、在整百价格水平出现的大幅离散跳跃后均值回归、basket套利
- 综合多种alpha策略回测约1.1-1.5M PnL
...
#algo-trading
- Tutorial两个资产：稳定型（stationary）spread较宽，适合做市，主动takeing机会几乎为零
- 漂移型资产上MR、趋势跟随、orderbook crossings都失败，复杂的regime detection/信号系统在该比赛中表现很差，简单策略反而更好
- 第二轮Osmium上"orderflow blips"信号被验证有效（订单流瞬时异动可作为入场信号）
- Pepper Root采用buy-and-hold简单策略
- 教训：MR策略要注意Z-score入场方向不要搞反

---

## 2026-04-30 09:44

- 单边订单簿（one-sided orderbooks）的"零"实际是套利机会：可以挂任意价格成交并降低库存
- Hydrogel Packs 适用简单均值回归策略效果不错
- 期权策略在有效市场下，Black-Scholes、IV、gamma scalping、做市等复杂方法都难以盈利，执行方式是关键
- 线性回归预测供需变化用于价格预测在本次比赛中无效
- 期权alpha与均值回归alpha合并使用反而互相干扰，效果变差

---

## 2026-04-30 09:49

- Round 5复盘：复杂的资产无关模型（自适应库存、报价规模、reservation price，结合相关性、均值回归、趋势信号）反而亏损-12K，简单的pairs trading加均值回归（类似Round 1、2）最赚钱；积极入场更有利
- Round 3/4 gamma scalping尝试：实现波动率明显高于IV，但12小时调试后仍难以稳定击败价差
- Manual trading解读：contamination事件视为临时供应中断，价格短期飙升后会回落

---

## 2026-04-30 09:59

- 算法交易：被动做市可在 mid±1 报价（best bid+1 / best ask-1），即使在标准 best bid/ask 也能成交，可观察到 bot 活跃度随回合变化
- 算法交易：可缩减 spread 至 2，或在 mid-price+1 处挂买单（让 bot 认为是好价），但成交率较低（每 tick 约 5-7 成交）
- 算法交易：第 3、4 轮期权策略——叠加 IV 相对 smile 拟合公允值的前 1% 偏离，可发现某些期权 IV 大幅偏离均值时对应当日价格高点，而较便宜的期权（如 5400、5500 行权价）对应当日低点，可作为均值回归信号

---

## 2026-04-30 10:04

- #algo-trading: 有选手通过捕捉与相关标的fair value的偏差进行交易，单轮获利34K
- #algo-trading: 第3-4轮使用硬编码价格区间进行均值回归（低于阈值买入、高于阈值卖出）即可获得80-90K PnL，复杂模型反而效果差
- #algo-trading: 提交前需检查硬编码值，避免遗忘导致大额亏损
- #manual-trading: Manual round中应避免全仓押注单一选项，建议对持仓进行对冲（如最多25%仓位），以防极端下行风险（历史上未见过-80%但仍需防范）
- #manual-trading: 解读manual题目时，百分比可能代表预期收益率（如16%意味着预期价格上涨32%），需仔细理解题意

---

## 2026-04-30 10:14

- algo-trading: 选手认为今年无法通过硬编码时间戳值获利，因为holdout数据的seed+噪声不同；过去能硬编码是因为数据复用了往年并通过bot行为推断出seed
- algo-trading: 有选手在均值回归(MR)策略中硬编码了指数衰减参数

---

## 2026-04-30 10:45

- Round 5策略思路：将产品按趋势聚类（3天内8个产品强趋势上涨、7个下跌），观察到某些产品的方向会在运行前20%时间段内根据其他产品的走势而改变
- Round 5观察：通过volume和不平衡交易的来源点位可进一步分析
- Sounds产品猜测：Solar flare可能呈现类似正弦的振荡模式

---

## 2026-04-30 10:50

- FFT分析显示在500000 ticks周期存在高振幅信号（来自#algo-trading，未能有效利用）
- 一种round 5策略组合：用残差作为信号、配对交易、以及基于EMA和Z-score的策略，平均收益约600k但担心过拟合
- 另一种思路是必须将产品与cluster相关联交易，但有8-9个产品难以找到相关性

---

## 2026-04-30 10:55

- #algo-trading: 有人尝试1-tick horizon的策略，但spread吃掉了所有收益，无法盈利
- #algo-trading: IV做市策略不可行，因为没人会接单
- #manual-trading: lava cake新闻分析——相比quantum coffee（仅潜在禁令就跌66%），lava cake有诉讼、销售停止、监管调查确认，跌幅可能更大，但保守估计更稳妥

---

## 2026-04-30 11:00

- #algo-trading: pebbles（VOLCANIC_ROCK相关产品）价格锚定在50000附近
- #algo-trading: Round 3期权交易中，有人用均值回归+布林带阈值策略，但买卖价差较大可能影响盈利
...
=== #algo-trading ===
- Round 5 voucher策略：绘制volatility smile与各voucher的偏差，筛选出每个voucher偏差最大的1%时点叠加到图表上，会发现昂贵的期权一致性出现在价格峰值，便宜的期权（5500和5400）一致性出现在低点；IV锚定在约5250，基于这些偏差进行交易，仓位大小很重要。该策略在Round 4获得130K PnL。

---

## 2026-04-30 11:15

- #algo-trading: 有选手认为波动率smile过于复杂，对简单市场来说只是噪声，简单的均值回归策略反而有效
- #algo-trading: fair value估计是普遍难点
- #algo-trading: 有人尝试将tick数据转为K线并基于candlestick形态交易，但供需类思路最终未盈利

---

## 2026-04-30 11:20

- 执行算法思路：基于发现的alpha调整fair value（FV = mid - inv + alpha），围绕FV做市报价，并在FV附近设置阈值进行taking
- 据观察，某选手的60分钟alpha实际是对某product的lead-lag硬编码，因此该product的order book很薄，lead-lag效应被削弱

---

## 2026-04-30 11:25

- 简单做市策略（best bid+1/best ask-1）配合调整目标库存的均值回归信号也能有效工作
- EWMA（指数加权移动平均）作为信号在比赛中表现意外地好
...
=== #algo-trading ===
- Pair trading在fruit snack（果零食类）产品上有效；其他microchips产品存在lead-lag关系
- Pebbles类产品适合做无风险的market making
- oxygen和robot类产品出现100价差时存在反向动量信号
- 用脚本扫描全部2^5×5×2^5组合，用ADF检验(p<0.05)寻找协整篮子，并用day2筛选、day3验证的holdout方法过滤噪声
- 同样方法用于lead-lag关系扫描（lag范围10到2000）
- 注意多重检验问题：50个产品约1250对，即使holdout也会有伪相关；通过加权组合能人为消除随机漂移造成假性平稳

=== #manual-trading ===
- 关注的是产品本身而非公司，供给冲击下公司股价跌但产品价格可能涨

---

## 2026-04-30 12:55

- Round相关：有团队提到未发现robot oscillation信号、错失pebbles机会
- 微观结构观察：有选手发现50个产品的最大bid-ask spread常出现在价格顶部附近，但因不是每天发生而未交易，可能存在潜在alpha信号

---

## 2026-04-30 13:10

- 价格序列中存在固定的fair value跳跃（如10和100），存在套利空间
- 策略思路：不假设趋势，将产品分类为OU过程、GBM过程或OU_GBM混合过程
- 观察发现pebbles XS和S呈下跌趋势，pebbles XL呈上涨趋势（但测试日不确定是否延续）
- 一种实现策略：找出11个组合使价格序列平稳，在-sigma处买入，+sigma处卖出（均值回归交易）

---

## 2026-04-30 13:15

- #algo-trading: 有选手发现某些产品组合相加严格等于50000（可能是basket/篮子套利关系），可用于降低方差至接近0
- #algo-trading: 该套利机会因bid-ask spread较大而难以实际交易获利
- #algo-trading: 行情中出现的尖刺（needle）可能是bug造成的，需谨慎对待

---

## 2026-04-30 13:20

- Round 3期权策略采用均值回归并调参，但回测表现不如频道中其他截图；提交时使用的是不同日期的数据
- 篮子产品（baskets）之间不存在真正的套利关系，篮子间的pair trading只是观察偏差/伪相关
- 平台上显示的portal成绩与本地backtester差异很大（portal普遍1M，本地差异显著）

---

## 2026-04-30 13:35

- 一种工作流程建议：第一天读取订单簿数据做alpha分析，写基础做市版本；用平台测试发现回测中不出现的特定bot；然后调参写策略
...
- algo-trading: 当指数（index）的卖价低于50时买入，是一个alpha策略，但触发频率不高

---

## 2026-04-30 14:00

- #algo-trading: 有人提到组合套利思路：Pebble XL + Chocolate + 黑洞 - 0.5 Pistachio ≈ 常数（疑似picnic basket类合成套利公式，需验证）

---

## 2026-04-30 14:25

- algo-trading: 对于某些产品，spread的计算公式为 `spread = int(mid_price/1250) + 2`，这不是alpha而是固定规律，无需在此浪费时间研究
...
- 某些产品价格变动呈±100跳跃模式，若上次+100，下次非零变动有85%概率为-100（具有均值回归特性）
- 上述跳跃规律并非始终有效，市场存在不同的regime切换
...
- rmtf1111团队对Round 5的策略：仅对历史数据中观察到信号的产品做反向动量逻辑，未对其他产品加额外逻辑（基于官方承诺数据分布不变）
- 对于历史价格波动在±100以内的产品，不假设其会出现反转动量行为

---

## 2026-04-30 15:20

- 在2500对的协整测试中，p-value=0.05下纯噪声约会有125对通过ADF检验，因此大量"通过"配对实际是假阳性
- 一阶差分相关性比协整检验更基础有效，但会漏掉microchips这类机会
- 日波动率(daily volatility)是识别合成数据alpha的关键指标，可以一图看出所有有效信号
- 由于数据是合成生成的，传统统计指标在此竞赛中表现良好
- 建议结合价格路径目视检查，过滤掉非OU过程的伪配对

---

## 2026-04-30 15:21

- 通过观察各产品波动率，发现波动率相同的是纯噪声生成的产品，而波动率不同的产品恰好是有alpha的产品
- 识别出有alpha的产品后，可依次进行一阶差分、自相关、领先-滞后相关性分析来挖掘信号
- 跨板块（cross sector）在某些标的上可能是错误的选择

---

## 2026-04-30 15:35

- snack packs与市场其他品种的一阶差分相关性约为0.22，有人尝试利用但未找到有效用法，社区中也未见明确的盈利策略

---

## 2026-04-30 15:45

- Pebbles (XS/S/XL) 的价格总和约为50k，但有±15左右的偏离，可作为潜在套利信号；不过有选手测试后发现实际效果不佳
- 对Pebbles的另一种思路是直接做市（market making）而非买卖持有

---

## 2026-04-30 18:42

- Round 4寻找alpha的思路：均值回归策略（有选手用此方法但仅获得约130K，未突破200K）

---

## 2026-04-30 18:45

- 对于有方向性现货信号的标的，可以利用期权作为杠杆放大收益

---

## 2026-04-30 18:47

- #algo-trading: 有选手发现d_iv（隐含波动率变化）具有均值回归特性，但因TTE推进导致BS模型参数变化，不确定期权本身是否仍呈现MR
- #algo-trading: 一个简单策略——现货做均值回归 + 期权仓位加倍，在r4取得约300k/220k的成绩，说明简单策略也能奏效

---

## 2026-04-30 19:02

- #algo-trading: 期权策略可以利用Black-Scholes从标的资产的z-score阈值反推到期权空间的IV阈值进行交易，比简单跟随标的同步下单更有效

---

## 2026-04-30 19:05

- 有人通过硬编码fair value进行交易，组织者表示基于数据分析的硬编码参数是允许的，只要不是预计算所有时间戳的最优交易
- 有选手在R3获得260/R4获得220 PnL（VR），另一选手300/220，改进版本R3达到330（发现HP在低/高excursion下行为不同）
- 期权lag为0的观察支持某算法的有效性
- IV反演（IV inversion）方法被认为更优

---

## 2026-04-30 19:07

- #algo-trading: 各期权的IV存在独立的均值回归特性，特别是VEV5000的IV明显围绕0.24均值回归，但因spread较大无法单独交易该IV信号
- #algo-trading: VEV5000可作为执行层的辅助手段，与标的的均值回归(MR)组件结合使用
- #algo-trading: 针对VEV5000设置独立阈值理论上可多带来约5k/日 PnL，是更优算法方案
- #algo-trading: 其他期权的IV看起来更像噪声，难以提取信号

---

## 2026-04-30 19:10

- #algo-trading: 有选手认为某期权的隐含波动率(IV)在0.24附近呈均值回归特性，且非滚动均值情形

---

## 2026-04-30 19:15

- osmium 通过 grid search 优化跟随微观结构的策略，但与顶尖选手的方法可能差异较大
- pepper 在 IMC 1k 网页提交中稳定获得 8200 PnL，进一步提升困难

---

## 2026-04-30 19:17

- #algo-trading: pepper产品需要找到空订单簿(empty order book)的漏洞利用
- #algo-trading: 期权交易中，若IV偏离均值与标的资产变动弱相关，则期权和标的的阈值应该大致相同；IV偏差范围[-2,2]但集中在0-0.5，难以覆盖价差，导致信号难以盈利

---

## 2026-04-30 19:20

- algo-trading: 价格偏差范围在-2到2之间，可考虑使用更小的spread配合更大的IV偏差进行交易
- algo-trading: 类似去年的情况，IV是主要alpha来源，但传闻存在rolling mean reversion，需识别哪个回归更快

---

## 2026-04-30 20:20

- 期权类策略思路：可以将期权价格建模为 X_t + Z_t，其中 X_t 是标的价格，Z_t 是均值回归项（本质上是线性分离的隐含波动率成分），可基于此构建交易信号

---

## 2026-04-30 20:23

- 关于多信号组合执行的讨论：当同时使用X_t本身的信号和Z_t=Y_t-X_t的价差信号时，两个信号可能产生冲突（如X_t低位做多，但Z_t突然下跌又要求做空），导致在底部反转加仓造成系统性亏损，单纯管理X_t仓位可能不是最优执行方式。
...
- 多时间尺度均值回归策略思路：当存在两个独立误差的均值回归信号（不同半衰期）时，可在短半衰期上更频繁交易，同时根据长半衰期价差的方向偏移库存，从而同时捕获两个尺度的价差；若两者不独立，则需要做变量变换后再交易。

---

## 2026-05-01 00:04

- Round 3-5策略思路：R3和R4使用均值回归；R5使用统计套利（相关性、lead-lag套利）+对特定资产做均值回归
- R5市场做市策略：有bot会一次性买卖所有持仓，可以通过抢先报价（beat the quoter）获取alpha
- R3关于Black-Scholes+gamma scalping的疑问：在提供的log中盈利好，但实盘过拟合导致负分（提示需注意过拟合风险）
- 回测与实盘存在显著差异（一选手BT 930k但实盘48k；另一选手BT 190k实盘130k）

---

## 2026-05-01 00:09

- Round 5 IV smile难以描述完整IV曲面，部分选手使用近似函数处理到期时间变化，部分选手直接拟合原始IV，但相比去年要复杂得多
- Round 5决定最终排名和奖金，IMC会联系获奖者验证身份防止作弊
...
- 分享了一个外部资源链接(prosperity.equirag.com)，包含Round 1到Round 4的相关内容

---

## 2026-05-01 09:26

- R4适合测试执行不变量（相比R3更好）；R5只能找到底层资产并做假设
- 评分运行始终是10k OOS数据

NO_INSIGHTS的判断：上述信息有一定参考价值，保留。

---

## 2026-05-01 09:30

- 网站测试数据集是第4天的前1/10部分（约1k条），而最终评测使用未知日期的完整10k条数据，但属于同一数据生成过程(DGP)
- 因此本地用历史数据回测足够，网站测试的主要价值在于验证执行层面（成交、流动性、bot行为）

---

## 2026-05-01 09:35

- Portal提交测试使用的是公开CSV中最后一天的前1000个ticks
...
- 有选手观察到50个证券中有40个同时执行交易的现象，并据此构建了"regime change"（市场状态切换）信号

---

## 2026-05-01 16:08

- algo-trading: 针对该轮比赛，建议先剔除会在"合理猜测"下产生剧烈盈利波动的随机种子，再在剩余种子上做优化；由于回报不对称，这种方法效果显著
- algo-trading: 该轮存在隐藏的"市场调整"环节会在事后改变结果，因此优化目标应考虑此不确定性，而非单纯追求最大化预期收益
